# PLS-DA: cancer (C) vs non-cancer (NC) from X_1–X_9
# 70:30 train/test split + 5-fold NESTED cross-validation
#
# NOTE ON "PLS-DA":
# scikit-learn has no ready-made PLS-DA class. PLS-DA = ordinary PLS regression
# run on numeric-coded labels (C -> 1, NC -> 0), with a threshold on top that
# turns the predicted number back into a class ("C"/"NC"). The little wrapper
# class below (PLSDAClassifier) does exactly that, so the rest of the pipeline
# can treat it like any other classifier (it has .predict, .predict_proba, etc.).
...
# ===========================================
# The PLS-DA "classifier" wrapper
# ===========================================
# PLS itself is a regression method: it predicts a number, not a label.
# This wrapper turns it into a classifier so it plugs into GridSearchCV etc.

class PLSDAClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, n_components=2, threshold=0.5):
        # n_components = the only real tuning knob of PLS (how many latent variables to keep).
        # This replaces all the RF hyperparameters.
        # threshold = the cut-off on the predicted score. score >= threshold -> "C".
        # 0.5 is the textbook default (labels are coded 0 and 1).
        self.n_components = n_components
        self.threshold = threshold

    def fit(self, X, y):
        # fix the class order so every part of the code agrees on it
        self.classes_ = np.array(["0", "1"])
        # turn the C/NC labels into numbers so PLS can do its (numeric) maths
        y_numeric = np.asarray(y, dtype=int)
        # scale=True standardises X and y internally (PLS needs this)
        self.pls_ = PLSRegression(n_components=self.n_components, scale=True)
        self.pls_.fit(X, y_numeric)
        return self

    def decision_score(self, X):
        # the raw continuous number PLS predicts (can be <0 or >1)
    ...
    def predict_proba(self, X):
        ...
        # only use the "C" column, and only for ranking (ROC/AUC).
        scores = self.decision_score(X)
        p_c = scores            # column for "C"
        p_nc = 1 - scores       # column for "NC"
        return np.column_stack([p_nc, p_c])


# VIP scores = "which X features matter most" (the PLS replacement for Gini importance)
def vip(pls_model):
    t = pls_model.x_scores_      # sample scores per component
    w = pls_model.x_weights_     # feature weights per component
    q = pls_model.y_loadings_    # y loadings per component
    p, h = w.shape               # p = number of features, h = number of components
    # how much y-variance each component explains (one number per component)
    ...
    for i in range(p):
        weight = np.array([(w[i, j] / np.linalg.norm(w[:, j])) ** 2
                           for j in range(h)])
        vips[i] = np.sqrt(p * float(np.sum(s * weight)) / total_s)
    return vips
...
# For PLS the ONLY thing to tune is n_components (number of latent variables).
...
grid = GridSearchCV(pls, param_grid, cv=inner_cv,
                    scoring=scorer, n_jobs=-1, refit='auc')
# pls = the estimator, param_grid = the hyperparameters to search, cv = the cross-validation strategy used to score each candidate,
# refit='auc' to decide which hyperparameter combination is the best, n_jobs=-1 uses all cores,
# and refits the best model on the whole training set after grid search.
...
print("(VIP > 1 is the usual rule-of-thumb for an 'important' feature)")
print()
